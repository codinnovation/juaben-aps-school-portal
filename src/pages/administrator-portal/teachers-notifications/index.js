...
function Index() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const dbRef = ref(db, "TeachersNotification");
  ...
  const handleSubmitNotification = async () => {
    try {
      const newNotification = push(ref(db, "TeachersNotification"), notificationData);
  ...
  const handleDeleteNotification = async (key) => {
    try {
      await remove(ref(db, `TeachersNotification/${key}`));
  ...
  const handleUpdateNotification = async () => {
    try {
      const notificationRef = ref(db, `TeachersNotification/${selectedNotification.key}`);
