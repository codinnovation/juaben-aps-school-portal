...
function StudentClassScore({ selectedStudent }) {
  ...
  const addSubjectScores = () => {
    if (user.displayName !== "Teacher") {
      toast.error("You dont have permission to Add Scores");
    } else {
      const scoreRef = ref(
        db,
        `/japsstudents/${selectedStudent.key}/Homework/${selectedSubject}`
      );
      push(scoreRef, {
        score: subjectScore,
        date: subjectDate,
  ...
  const fetchSubjectScores = () => {
    const studentRef = ref(db, `/japsstudents/${selectedStudent.key}/Homework`);
    get(studentRef)
      .then((snapshot) => {
        if (snapshot.exists()) {
      ...
      <div className={styles.classScoreContainer}>
        <div className={styles.classScoreItems}>
          <div className={styles.subjectHeader}>
            <h1>{`Home work - ${selectedSubject}`}</h1>
          </div>
